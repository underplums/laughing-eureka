...
WITH dac_history AS (
    SELECT 
        contact_id,
        date_month,
        date_month - (ROW_NUMBER() OVER(PARTITION BY contact_id ORDER BY date_month) * INTERVAL '1 month') AS grp
    FROM dm.prvdr_dac_monthly
    WHERE has_transaction_activity = 1
      AND (
          has_mobapp_activity = 1
          OR has_pwa_activity = 1
          OR vcoff_trn_cnt > 0
      )
      AND date_month < date_trunc('month', date'{date}') -- Считаем только историю до целевого месяца
),
islands AS (
    SELECT 
        contact_id,
